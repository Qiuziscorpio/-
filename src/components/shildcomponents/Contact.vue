  ...
  <div class="pane-C">
    ...
    <div class="panel-C-content">
        ...
        <router-link :to="{name:contact.routername,params:{id:contact.Id}}"tag="div" class="shop" v-if="contact.routername==='supplierdetail'">
          <i class="icon iconfont icon-weishoucang text-yellow" v-if="contact.IsFav===false"></i> 
          <i class="icon iconfont icon-yishoucang text-yellow" v-else></i>
          收藏店铺
        </router-link>
        <router-link :to="{name:contact.routername,params:{id:contact.Id}}"tag="div" class="shop" v-else>
